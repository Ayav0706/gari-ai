...
export async function listMemories(userId: number): Promise<MemoryEntry[]> {
    const snapshot = await db.collection('users').doc(userId.toString())
        .collection('memories').orderBy('updated_at', 'desc').get();
...
export async function getMemorySummary(userId: number): Promise<string> {
    const snapshot = await db.collection('users').doc(userId.toString())
        .collection('memories').orderBy('updated_at', 'desc').limit(20).get();
    if (snapshot.empty) return "";
    return snapshot.docs
        .map(doc => { const data = doc.data(); return `\u2022 ${data.key}: ${data.value}`; })
        .join("\n");
}
...
export async function getRecentMessages(
    userId: number, limit: number = 20
): Promise<{ role: string; content: string }[]> {
    const snapshot = await db.collection('users').doc(userId.toString())
        .collection('conversations').orderBy('timestamp', 'desc').limit(limit).get();
